...
The methodology of the AI training will be similar to training it to play a game. Each episode of data will be fed to the AI one row at a time, along with SOME of the meta data at the top of the 5 min episode. The AI should not be fed information it isn't supposed to know yet. After the AI receives a line of input, it has essentially nine choices, in three categories.
    ...
    ### Fill a currently active order (taker).
        - Buy five "up" shares at the current ask price.
        - Sell all held "up" shares at the current bid price.
        - Buy five "down" shares at the current ask price.
        - Sell all held "down" shares at the current bid price.

    ### Enter an order in the order book (maker).
        - Enter a limit buy order for five "up" shares at one penny below the current ask price.
        - Enter a limit sell order for all held "up" shares at one penny above the current bid price.
        - Enter a limit buy order for five "down" shares at one penny below the current ask price.
        - Enter a limit sell order for all held "down" shares at one penny above the current bid price.
        ...
        - Once a limit order is entered, the agent may only "do nothing" until the order fills or the episode ends. Limit orders are considered filled if, in a future row during that episode, the market price reaches the order price. For example, if the agent entered a limit sell order at 4 cents, and there is a later row where the bid for that share is 4 cents or higher, then the order fills at the limit price. If the order is not filled by the end of the episode, the order is cancelled and has no cash effect.

    ### Multi-trade mechanics
        - The agent alternates between buy mode and sell mode. It starts each episode in buy mode (no shares held).
        - In buy mode, only buy actions are available. Once the agent buys (or a limit buy fills), it enters sell mode.
        - In sell mode, only sell actions are available for the direction of shares held. Once the agent sells all shares (or a limit sell fills), it returns to buy mode.
        - This buy→sell→buy cycle can repeat multiple times within a single episode, one action per row.
        - The agent may not buy additional shares while already holding shares.
        - All buys are for a fixed quantity of 5 shares. Fees (taker) or rebates (maker) are absorbed into the share count rather than tracked as separate cash flows. For example, buying 5 shares at 10¢ taker costs 50¢ cash and yields slightly fewer effective shares after the fee.
        - An `is_sell_mode` flag (1.0 when holding shares, 0.0 otherwise) is included in the observation so the agent knows which mode it is in.

        - At the end of the episode, the AI will receive $1 per share if it holds shares on the winning side (bought UP and outcome is UP, or bought DOWN and outcome is DOWN). If it holds shares on the losing side, it receives nothing. Any unsold shares are paid out at resolution.
        - Any pending unfilled limit order at episode end is cancelled with no cash effect.
        - The total profit or loss for the episode is the sum of all sell proceeds minus all buy costs, plus any end-of-episode share payout.

        - All bids and asks are in cents; Bitcoin prices are in dollars.

        - There are nulls in the data. If a null occurs in a bid or ask price, that action is not available to the AI.

    ### Reward function
        - The final episode profit/loss is the reward signal. It is computed as `(net_cash + end_payout) / 500` where 500 is a normalization constant (5 shares × $1 maximum payout × 100¢/$).
        - To give the agent consistent feedback at every timestep, the terminal episode reward is divided evenly across all rows in the episode. Every row receives `episode_reward / N` where N is the total number of rows. This means a winning episode produces small positive rewards at every step, and a losing episode produces small negative rewards at every step — matching the true outcome polarity throughout training.
        - Episodes where the agent does nothing at all have a terminal reward of 0, so all their per-step rewards are also 0.
        - Fees and rebates modify the effective share count at purchase time and thus affect the final P&L.
